...
- the package-local verify entrypoint now fails closed when any queue mirror repeats the pinned M113 package row or when any registry mirror repeats task `113.4`, so first-match block extraction cannot mask duplicate closure evidence.
