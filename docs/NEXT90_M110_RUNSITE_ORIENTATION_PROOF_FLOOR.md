...
- `tests/test_runsite_orientation_bundle_contracts.py` fail-closes contract drift so runsite orientation stays first-class and render-verified.
...
- package authority must keep exactly one canonical M110 queue row in each queue mirror and exactly one `110.2` registry task block
