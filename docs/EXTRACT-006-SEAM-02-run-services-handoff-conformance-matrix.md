...
| --- | --- | --- | --- | --- |
| M-ING-01 | Common render intake (`run-services` -> `media-factory`) | `MediaRenderRequest` with `renderKind`, `requestId`, `requestedAtUtc`, deterministic `idempotencyKey`, and render-verified `inputs` map | run-services constructing provider-native request bodies or SDK payloads | run-services submits only `Chummer.Media.Contracts` request DTOs; provider payload expansion happens inside media-factory executors |
