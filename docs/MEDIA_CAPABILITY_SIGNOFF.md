...
- structured media recipes via `StructuredMediaRecipeRequest`, `StructuredMediaRecipeBundleReceipt`, `StructuredMediaRecipeArtifactReceipt`, `StructuredMediaRecipePublicationRefReceipt`, `StructuredMediaRecipeCaptionRefReceipt`, `StructuredMediaRecipePreviewRefReceipt`, `StructuredRecipeVideo`, `StructuredRecipeAudio`, `StructuredRecipePreviewCard`, and `StructuredRecipePacketBundle`
...
- structured media recipe receipts carry publication-ready refs, caption refs, preview refs, first-class `PublicationRefReceipts`, `CaptionRefReceipts`, `PreviewRefReceipts`, role-specific receipt ids, and media-factory job ids for video, audio, preview-card, and packet-bundle siblings

## Release statement

Media completion is now good enough for program closure because documents, portraits, bounded video, route artifacts, structured media recipe bundles, and archive/retention all have explicit owner contracts, lifecycle governance, and executable verification. Future provider expansion is additive depth, not missing service ownership.
