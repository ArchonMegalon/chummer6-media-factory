...
M115_REPLAY_EXCHANGE_PREVIEW_PACKAGE = {
...
M116_CREATOR_PROMO_KIT_PACKAGE = {
...
M135_MEDIA_COVERAGE_PACKAGE = {
    ...
    "task": "Close media contracts, render jobs, previews, manifests, archive, provider adapters, and asset lifecycle coverage.",
    "work_task_id": "135.9",
    ...
    "proof": [
        "src/Chummer.Media.Contracts/Compatibility/RunServices/MediaFactoryContracts.cs",
        "src/Chummer.Media.Contracts/Rendering/MediaRenderKind.cs",
        "src/Chummer.Media.Factory.Runtime/Assets/MediaRenderJobService.cs",
        "src/Chummer.Media.Factory.Runtime/Assets/StructuredMediaRecipeExecutionService.cs",
        ...
        "docs/NEXT90_M119_STARTER_ARTIFACT_PROOF_FLOOR.md",
        "docs/NEXT90_M145_EXPLAIN_PRESENTER_SIBLINGS_PROOF_FLOOR.md",
        "docs/NEXT90_M135_MEDIA_COVERAGE_PROOF_FLOOR.md",
        "tests/test_m135_media_coverage_proof.py",
        "scripts/ai/materialize_media_release_proof.py",
        "scripts/ai/verify_m135_media_coverage.sh",
        "scripts/ai/verify.sh",
    ],
}
...
def require_unique_successor_package_ids(packages: list[Dict[str, Any]], contract_name: str) -> None:
    ...
    for package in packages:
        package_id = str(package["package_id"]).strip()
        counts[package_id] = counts.get(package_id, 0) + 1

    duplicates = sorted(package_id for package_id, count in counts.items() if count > 1)
    if duplicates:
        raise ValueError(
            f"{contract_name} must contain exactly one successor package entry per package id; duplicates: {', '.join(duplicates)}"
        )
...
def main() -> int:
    ...
    require_unique_successor_package_ids(
        ...
        media_release_proof["contract_name"],
    )
    require_unique_successor_package_ids(
