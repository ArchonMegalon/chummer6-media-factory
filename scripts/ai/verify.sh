...
rg -n 'DocumentPdf|DocumentThumbnailImage|PortraitImageVariant|NarrativeBriefVideo|RouteCinemaResult|StructuredMediaRecipeBundleReceipt|StructuredRecipeVideo|StructuredRecipeAudio|StructuredRecipePreviewCard|StructuredRecipePacketBundle|AssetLifecyclePolicy|CHUMMER_MEDIA_FACTORY_IMAGE_BACKEND|CHUMMER_MEDIA_FACTORY_ENABLE_IMAGE_EXECUTION' docs/MEDIA_CAPABILITY_SIGNOFF.md >/dev/null
rg -n 'StructuredMediaRecipeExecutionService|PublicationRefs|CaptionRefs|PreviewRefs|VideoReceiptIds|AudioReceiptIds|PreviewReceiptIds|PacketReceiptIds|PublicationRefReceipts|CaptionRefReceipts|PreviewRefReceipts' src docs/MEDIA_CAPABILITY_SIGNOFF.md >/dev/null
...
python3 -m unittest tests/test_structured_media_recipe_execution.py
